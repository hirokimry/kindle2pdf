...
│  ├─ preprocess.py          # トリミング・正規化（1 撮影 = 1 ページ）
...
capture:
  ...
  # 見開きの左右分割は廃止（Issue #29）。見開き/片ページは Kindle のウィンドウ幅で選ぶ（半画面幅=片ページ / 全画面幅=見開き 1 枚）。撮影は常に 1 撮影 = 1 ページ
...
ocr:
  ...
  reading_order: "rtl"            # 見開き2カラムの読み順方向: rtl(右→左・漫画/縦書き) | ltr(左→右・横書き)
...
```json
{
  ...
  "pages_total": 84,               // 確定ページ数（1 撮影 = 1 ページ・preprocess後に確定）
...
def process_all(cfg, state) -> None           # raw/ -> pages/（1 撮影 = 1 ページ・分割しない）
```

### 5.4 `ocr.py`

```python
def order_reading_items(items, reading_order) -> list  # 見開き2カラムを列認識で読み順に並べ替え
def ocr_page(path, cfg) -> list[tuple[str, float, list[float]]]:
    """engineで backend を選び→読み順ソート。返り値: (text, confidence, [x,y,w,h]) 正規化・原点左下。"""
    items = _ocr_page_google(path) if cfg.ocr.engine == "google" else _ocr_page_apple(path, cfg)
    return order_reading_items(items, cfg.ocr.reading_order)
def _ocr_page_apple(path, cfg)   # ocrmac(Apple Vision)。既定・端末内・macOS専用
def _ocr_page_google(path)       # urllibでVision REST。鍵はGOOGLE_VISION_API_KEY。応答は純関数でパース
```
**PoC根拠**: Apple Visionは余分な空白がほぼ無く（0.01/字）、日本語の語句検索が壊れない。読み順ソート（`order_reading_items`）は engine 非依存で dispatch 側が一度だけ適用する。見開き分割廃止（Issue #29）で 1 ページが 2 カラムになり得るため、中心線をまたぐ全幅行が無ければ左右の列に分けて各列を上→下に並べ、`reading_order`（rtl/ltr）の向きで連結して読み順の破綻を防ぐ。**engine=google**（Issue #56）はオプトインで Google Cloud Vision（手描き/崩し字に強いがクラウド送信・従量課金）、既定は `apple`。
...
| P4 | トリミング（1 撮影 = 1 ページ） | `raw/`→`pages/` | 各`pages/`がUI無し・ウィンドウ中身のまま（分割しない） |
...
**ゴールデンテスト**: 本日のPoCスクショ4枚を`tests/fixtures/`に置き、P4〜P6の期待出力（確定ページ数、既知語の検索ヒット、座標が本文域内）を回帰テスト化する。
...
- **見開き読み順**: 左右分割を廃止した（Issue #29）ため 1 ページが 2 カラムになり得る。`order_reading_items()` が Vision bbox の `x` で列認識し、中心線をまたぐ全幅行が無ければ左右の列に分けて各列を y ソートし、`reading_order`（rtl/ltr）の向きで連結する。全幅行があれば片ページ（単一カラム）とみなし上→下ソートのみに退避する。
...
- **縦書き（将来）**: 縦書き書籍はVisionが向き検出。`reading_order` を縦書き用に拡張する余地を残す。

---

## 🔐 8. セキュリティ姿勢（設計として明記）

このツールがmacOSに要求する強い権限は**アクセシビリティ（キー送出）だけ**で、それも**ユーザー自身のTerminalに、実行時に限って**付与される。撮影・OCR・PDF化はいずれも特別権限不要。

**既定はすべてローカル完結で外部送信なし。** 例外は OCR エンジンに `google`（Google Cloud Vision）を明示選択した場合のみで、そのとき **書籍ページ画像が Google のサーバーへ送信される**（オプトイン・従量課金・Issue #56）。既定の `apple`（端末内 Apple Vision）では従来どおり外部送信は発生しない。個人が購入した書籍の私的利用に限定する前提は仕様書0章の通り。
