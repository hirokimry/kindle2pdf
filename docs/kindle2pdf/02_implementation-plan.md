...
def ocr_page(path, cfg) -> list[tuple[str, float, list[float]]]:
    """ocrmacでVision OCR→読み順ソート。返り値: (text, confidence, [x,y,w,h]) 正規化・原点左下。"""
    from ocrmac import ocrmac
    result = ocrmac.OCR(path,
        language_preference=cfg.ocr.languages,
        recognition_level=cfg.ocr.recognition_level).recognize()
    return order_reading_items([...], cfg.ocr.reading_order)
```
**PoC根拠**: Visionは余分な空白がほぼ無く（0.01/字）、日本語の語句検索が壊れない。見開き分割廃止（Issue #29）で 1 ページが 2 カラムになり得るため、中心線をまたぐ全幅行が無ければ左右の列に分けて各列を上→下に並べ、`reading_order`（rtl/ltr）の向きで連結して読み順の破綻を防ぐ。
