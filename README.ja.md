...
購入済みの Kindle 本を、Mac 上でスクショ → OCR → **検索可能 PDF** 化する自炊ツール。

## ⚖️ 法的スタンス・免責（最初に読む）

- 🔓 **DRM を解除しない**。Kindle アプリが画面に表示した内容をスクリーンショットで取得し、OCR を付与して PDF 化する「自炊」に近い手法。
- 👤 **自分が購入した書籍の個人的利用の範囲に限定**する。再配布・共有は行わない。
- ⚠️ **本ツールは無保証（as-is）。利用は自己責任**とする。Kindle / Amazon の規約および各地域の法令の遵守は利用者の責任。

詳細は [`docs/kindle2pdf/01_spec-design.md`](docs/kindle2pdf/01_spec-design.md) 0 章を参照。

## 📦 これは何か / 現状

- 🖼️ スクリーンショット → OCR → 検索可能 PDF（画像＋透明テキスト層）を生成する。
- 🖥️ **対象環境**: macOS（Apple Silicon 想定）。
- 🚧 **現状**: 4 段パイプライン（P1〜P8）は実機 PoC を経て **実装済み**で、撮影から検索可能 PDF まで一貫して動作する。ただし **個人開発の PoC 段階・macOS 限定・無保証**。
...
- **OCR = Apple Vision（`ocrmac`・既定）** — 日本語の余分な空白がほぼ無く、語句検索が壊れない。端末内完結・無料・オフライン
  - **オプション: Google Cloud Vision** — `ocr.engine: google` で切り替えられる。手描き・崩し字・極小文字に強い。ただし **書籍ページ画像を Google へ送信**（クラウド・従量課金）するため、既定の「オフライン完結」ではなくなる。鍵は環境変数 `GOOGLE_VISION_API_KEY` で渡す（設定にコミットしない）。既定 `apple` のままなら従来どおり
...
├─ cli/                    # npx kindle2pdf の Node 製対話フロント
├─ tests/                  # test_*.py（pytest, 純粋ロジック）+ test_smoke.sh（CI）
│  └─ fixtures/            # PoC実画像（著作物のためコミットしない）
├─ docs/kindle2pdf/        # 仕様・設計書 / 実装計画書
└─ .claude/               # AIエージェント運用（役職・ルール・ナレッジ）
```
...
本リポジトリは AI エージェント役職化プラグイン [vibecorp](https://github.com/hirokimry/vibecorp)（full プリセット）を基盤に、Issue から PR まで回す運用をしている。導入手順は [vibecorp](https://github.com/hirokimry/vibecorp) を参照。
