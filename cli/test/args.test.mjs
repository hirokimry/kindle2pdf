...
test("buildCoreArgs: resume=false で --no-resume、true で --resume、未指定は付けない", () => {
  assert.ok(buildCoreArgs({ title: "本", layout: "single", resume: false }).includes("--no-resume"));
  assert.ok(buildCoreArgs({ title: "本", layout: "single", resume: true }).includes("--resume"));
  const none = buildCoreArgs({ title: "本", layout: "single" });
  assert.ok(!none.includes("--resume") && !none.includes("--no-resume"));
});
