...
- **OCR = Apple Vision (`ocrmac`, default)** — almost no stray spaces in Japanese text, so phrase search does not break. On-device, free, offline.
  - **Optional: Google Cloud Vision** — switch with `ocr.engine: google`. Stronger on handwriting / stylized / tiny text, but it **sends the book page images to Google** (cloud, metered billing), so it is no longer offline-only. The key is passed via the `GOOGLE_VISION_API_KEY` environment variable (never committed to config). Leaving the default `apple` keeps the previous behavior.
